...
void calc_mv_init(void)
{
  ...
  xv_mvdata[2].start = xv_mvdata[2].out_old = xv_mvdata[2].pos_old = xp_mv_straight.z3;
  xv_mvdata[5].start = xv_mvdata[5].out_old = xv_mvdata[5].pos_old = xp_mv_straight.z3;

  xv_odometry.moveX = 0.f;
  xv_odometry.moveY = 0.f;
  xv_odometry.rotZ = 0.f;
  xv_odometry.x[0] = xv_odometry.x[1] = xv_odometry.x_old[0] = xv_odometry.x_old[1] = xv_odometry.y[0] =
      xv_odometry.y[1] = xv_odometry.y_old[0] = xv_odometry.y_old[1] = xv_odometry.theta[0] = xv_odometry.theta[1] =
          xv_odometry.theta_old[0] = xv_odometry.theta_old[1] = 0.f;
}
...
void calc_mv(void)
{
  ...
  if (mode_motion != MOTION_MOTION)
  {
    ...
    xv_posture.pitch = calc_mvdata(&xv_mvdata[6], &xv_data_pitch);
    /*	leg pitch			*/  // TODO: 今は足首のピッチになっていない．修正必要
                          //		xv_posture.pitch	=	xv_data_pitch.pos;								/*	leg pitch			*/		// TODO:
                          //今は足首のピッチになっていない．修正必要
  ...
  if (flag_walk.upleg == LEFT)
  {
    ...
    _tmp_y = ((xv_odometry.y[0] - xv_odometry.y[1]) - (xv_odometry.y_old[0] - xv_odometry.y_old[1])) *
             odometry_correct_para_y;  //横方向の振動抑制　14.06.07
  }
  else
  {
    /*	tracks of left leg	*/
    ...
    _tmp_y = ((xv_odometry.y[1] - xv_odometry.y[0]) - (xv_odometry.y_old[1] - xv_odometry.y_old[0])) *
             odometry_correct_para_y;  //横方向の振動抑制　14.06.07
  }
