...
void acc_fun(void)
{
  ...
  TIMER(xv_acc.acc_pitch >= xp_acc.fall_fwd, flag_acc.fall_fwd_on, xp_acc.fall_check_time, xv_acc.fall_fwd_work);

  TIMER(xv_acc.acc_pitch <= xp_acc.fall_bwd, flag_acc.fall_bwd_on, xp_acc.fall_check_time, xv_acc.fall_bwd_work);

  TIMER(xv_acc.acc_roll >= xp_acc.fall_right, flag_acc.fall_right_on, xp_acc.fall_check_time, xv_acc.fall_right_work);

  TIMER(xv_acc.acc_roll <= xp_acc.fall_left, flag_acc.fall_left_on, xp_acc.fall_check_time, xv_acc.fall_left_work);
  ...
  if (xv_acc.acc_roll >= xp_acc.fall_roll || xv_acc.acc_roll <= -xp_acc.fall_roll)
  {
    ...
    flag_acc.fall = 1;
           ...
           ((xv_acc.acc_pitch <= -xp_acc.fall_pitch_oblique) && (fabs(xv_acc.acc_roll) >= xp_acc.fall_roll_oblique)))
    flag_acc.fall = 2;
  else if ((fabs(xv_acc.acc_pitch) < 0.5 * xp_acc.fall_pitch) && (fabs(xv_acc.acc_roll) < 0.5 * xp_acc.fall_roll))
  {
    flag_acc.fall = 0;
...
void acc_init(void)
{
  ...
  xv_acc.acc_data1 = xv_acc.acc_data2 = xv_acc.acc_data3 = xv_acc.acc_data1_d = xv_acc.acc_data2_d =
      xv_acc.acc_data3_d = xv_acc.acc_data1_flt = xv_acc.acc_data2_flt = xv_acc.acc_data3_flt = xv_acc.acc_pitch =
          xv_acc.acc_roll = xv_acc.acc_pitch_d = xv_acc.acc_roll_d = 0.;
  xv_acc.fall_fwd_work = xv_acc.fall_bwd_work = xv_acc.fall_right_work = xv_acc.fall_left_work = 0;
  flag_restart_work = 0;
  flag_ukemi_finished_work = 0;
  flag_motion_accept_work = 0;

  flag_acc.zero = flag_acc.fall_fwd_on = flag_acc.fall_bwd_on = flag_acc.fall_right_on = flag_acc.fall_left_on =
      flag_acc.fall = flag_acc.standup_select = OFF;
  flag_restart = OFF;
  flag_ukemi_finished = OFF;
  flag_ukemi_start = OFF;
  flag_servo_restart = OFF;
