  ...
  switch (mode_sq_motion)
  {
      ...
      for (i = 0; i <= 28; i++)
      {
        if (i != HEAD_YAW && i != HEAD_PITCH)
        {
          if (flag_variable_motion == ON)
          {
            ...
            xv_data_d[i].pos =
                xp_mv_motionbuf[n][sq_motion_mtn].d[i] + xp_mv_motionbuf_var[n][sq_motion_mtn].d[i] * variable_amount;
    ...
    case SQ_MODE2_WAIT: /* wait */
      ...
      if (mode_sq_time >= (xp_mv_motionbuf[n][sq_motion_mtn].time / 100.f - EPS_TIME))
      {
        ...
        else if (!flag_motion_repeat ||
                 (xp_mv_motionbuf[n][sq_motion_mtn].cntr[0] == xp_mv_motionbuf[n][sq_motion_mtn].cntr[1]))
        {
          if (xp_mv_motionbuf[n][sq_motion_mtn].cntr[0] >= 1 &&
              xp_mv_motionbuf[n][sq_motion_mtn].cntr[0] <= MODE2_MOTION_SIZE)
          {
        ...
        {
          if (xp_mv_motionbuf[n][sq_motion_mtn].cntr[1] >= 1 &&
              xp_mv_motionbuf[n][sq_motion_mtn].cntr[1] <= MODE2_MOTION_SIZE)
          {
            ++count_motion_repeat;
