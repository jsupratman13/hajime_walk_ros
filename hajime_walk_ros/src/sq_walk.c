...
int sq_walk(void)
{
  ...
  {
    xv_mv_walk.x_percent_dlim =
        dlim_wait_fun(&xp_dlim_wait_x, &xv_dlim_wait_x);  //歩幅の割合(0-1) 徐々に歩幅を増やすための仕組み
  ...
  */
  ...
  if (xv_mv_walk.x_percent_dlim > EPS_DATA)
  { /*	walk forward	*/
    if (accurate_one_step_mode == 1)
    {
  ...
  else if (xv_mv_walk.x_percent_dlim < -EPS_DATA)
  { /*	walk backward	*/
    float w1, w2, w3;
  ...
  if (xv_mv_walk.y_percent_dlim > EPS_DATA)
  {  // ■右に横歩きする場合
    flag_walk.y = RIGHT;
  ...
  else if (xv_mv_walk.y_percent_dlim < -EPS_DATA)
  {  // ■左に横歩きする場合
    flag_walk.y = LEFT;
    flag_walk.turn = LEFT;
    if (accurate_one_step_mode == 1)
  ...
  {  // ■真っ直ぐ歩く場合
    ...
    if (xv_mv_walk.theta_percent_dlim > EPS_DATA)
    {  // ■右に旋回
      flag_walk.turn = RIGHT;
    }
    else if (xv_mv_walk.theta_percent_dlim < -EPS_DATA)
    {  // ■左に旋回
      flag_walk.turn = LEFT;
    }
    else
  ...
  switch (mode_sq_walk)
  {
    case SQ_WALK_INIT: /*	init	*/
      ...
      side_step_modify(t1, t2, t1a, t1b, t1c, t2a,  // 横歩きのためのパラメータ修正
                       &t1_r, &t2_r, &t1a_r, &t1b_r, &t1c_r, &t2a_r, &t1_l, &t2_l, &t1a_l, &t1b_l, &t1c_l, &t2a_l,
    ...
    case SQ_WALK_MV_L: /*	move cog to left	*/
      ...
      xv_data_y_r.time = xv_data_y_l.time = t2 * xp_mv_walk.start_time_k1;
      ...
      if (++xv_mv.count >= xv_mv_walk.num)
        ...
        mode_sq_walk = SQ_WALK_UP_R0; /*	continue		*/
    ...
    case SQ_WALK_MV_R: /*	move cog to right	*/
      ...
      if (++xv_mv.count >= xv_mv_walk.num)
        mode_sq_walk = SQ_WALK_UP_L2;
      else
        mode_sq_walk = SQ_WALK_UP_L0;

      break;

    case SQ_WALK_UP_R0: /*	up right leg for first step	*/
      if (mode_sq_time < (t2 * xp_mv_walk.start_time_k1 - EPS_TIME))
        break; /*	wait until previos state finish	*/
    ...
    case SQ_WALK_UP_R_1: /*	up right leg	*/
      if (mode_sq_time < (t1b_r - EPS_TIME))
        break;  // 両足支持期間が終了するまで，次に進まない
      ...
      work = (accurate_one_step_mode == 1 ? -xv_mv_walk.accurate_step_z : -xv_mv_walk.z) +
             xp_mv_ready.z3;                        // レディの高さから足上げ量を引いた値
      ...
      // 腕振り
      xv_data_d[ARM_PITCH_R].time = xv_data_d[ARM_PITCH_L].time = xv_data_d[ELBOW_PITCH_R].time =
    ...
    case SQ_WALK_DW_R: /*	down right leg	*/
      if (mode_sq_time < (t2a_r - RTC_TIME_SEC - EPS_TIME))
        break;  // 足を上げるまで先に進まない
    ...
    case SQ_WALK_MV_R2: /*	move cog to right	*/
      if (mode_sq_time < (t2a_r - EPS_TIME))
        break;  // 足を下げるまで先に進まない
    ...
    case SQ_WALK_UP_L_1: /*	up left leg		*/
      if (mode_sq_time < (t1b_l - EPS_TIME))
        break;  // 両足支持期間が終了するまで，次に進まない
    ...
    case SQ_WALK_DW_L: /*	down left leg		*/
      if (mode_sq_time < (t2a_l - RTC_TIME_SEC - EPS_TIME))
        break;  // 足を上げるまで先に進まない
    ...
    case SQ_WALK_MV_L2: /*	move cog to left	*/
      if (mode_sq_time < (t2a_l - EPS_TIME))
        break;  // 足を下げるまで先に進まない
    ...
    case SQ_WALK_UP_R2_1: /*	up right leg for last step	*/
      ...
      xv_data_d[ARM_PITCH_R].time = xv_data_d[ARM_PITCH_L].time = t1_r;
      xv_data_d[ARM_PITCH_R].pos = xp_mv_ready.arm_sh_pitch - xv_mv_walk.arm_sh_pitch;
      xv_data_d[ARM_PITCH_L].pos = xp_mv_ready.arm_sh_pitch + xv_mv_walk.arm_sh_pitch;
    ...
    case SQ_WALK_UP_R2_2: /*	up right leg for last step	*/
      xv_data_d[LEG_YAW_R].time = xv_data_d[LEG_YAW_L].time = t1a_r;
    ...
    case SQ_WALK_DW_R2: /*	down right leg for last step	*/
      ...
      xv_data_y_r.time = xv_data_y_l.time = t1_r;
    ...
    case SQ_WALK_UP_L2_1: /*	up left leg for last step	*/
      ...
      xv_data_d[ARM_PITCH_R].time = xv_data_d[ARM_PITCH_L].time = t1_l;
      xv_data_d[ARM_PITCH_R].pos = xp_mv_ready.arm_sh_pitch + xv_mv_walk.arm_sh_pitch;
      xv_data_d[ARM_PITCH_L].pos = xp_mv_ready.arm_sh_pitch - xv_mv_walk.arm_sh_pitch;
    ...
    case SQ_WALK_UP_L2_2: /*	up left leg for last step	*/
      xv_data_d[LEG_YAW_R].time = xv_data_d[LEG_YAW_L].time = t1a_l;
    ...
    case SQ_WALK_DW_L2: /*	down left leg for last step	*/
      ...
      xv_data_y_r.time = xv_data_y_l.time = t1_l;
      xv_data_y_r.pos = xv_mv_walk.zmp + xp_mv_walk.y_wide;
      xv_data_y_l.pos = xv_mv_walk.zmp - xp_mv_walk.y_wide;
    ...
    case SQ_WALK_READY:    /*	ready position	*/
      ...
      if (mode_sq_time < (t2a - RTC_TIME_SEC * 2 - EPS_TIME))
        break;
      ...
      xv_data_d[ARM_PITCH_L].time = xv_data_d[ARM_PITCH_R].time = xv_data_d[ARM_ROLL_L].time =
          xv_data_d[ARM_ROLL_R].time = xv_data_d[ELBOW_PITCH_L].time = xv_data_d[ELBOW_PITCH_R].time = t1;

      xv_data_d[ARM_PITCH_L].pos = xv_data_d[ARM_PITCH_R].pos = xp_mv_ready.arm_sh_pitch;
