...
void set_xv_comm(tp_xv_comm* xv, unsigned char cmd, unsigned char para1, unsigned char para2, unsigned char para3,
  ...
  switch (_xv_comm.cmd)
  {
    ...
    case 'R': /*	status of robot	*/
      ...
      if (flag_moving == STATE_WALKING)
        joy_status.para1 = '1'; /*	walking					*/
      else if (flag_moving == STATE_MOTION)
        joy_status.para1 = '2'; /*	acting special action	*/
      else if (flag_moving == STATE_MOVING)
        joy_status.para1 = '3'; /*	moving					*/
      else
        joy_status.para1 = '0'; /*	stop					*/

      /*	fall down flag	*/
      joy_status.para2 = '0';
      if (flag_gyro.fall == 1)
        joy_status.para3 = '1'; /*	front	*/
      else if (flag_gyro.fall == 2)
        joy_status.para3 = '2'; /*	back	*/
      else if (flag_gyro.fall == 3)
        joy_status.para3 = '3'; /*	right	*/
      else if (flag_gyro.fall == 4)
        joy_status.para3 = '4'; /*	left	*/
      else
        joy_status.para3 = '0'; /*	not fallen	*/
    ...
    case 'r': /*	status of robot	(read voltage) */
      ...
      if (flag_acc.fall == 1)
        joy_status.para3 = '1'; /*	front	*/
      else if (flag_acc.fall == 2)
        joy_status.para3 = '2'; /*	back	*/
      else if (flag_acc.fall == 3)
        joy_status.para3 = '3'; /*	right	*/
      else if (flag_acc.fall == 4)
        joy_status.para3 = '4'; /*	left	*/
      else
        joy_status.para3 = '0'; /*	not fallen	*/
...
//			old_yaw = xv_gyro.gyro_yaw2;
    ...
    case 'A': /*	walk all direction	*/
      ...
      /***	number of steps	***/
      if (!sq_flag.walk)
        xv_mv.count = 0;  // ���s���Ă��Ȃ����ɂ́C�����J�E���^��0�ɂ���D
    ...
    case 'H': /*	head yaw	*/
      ...
      if (abs(w) < 150.f && _xv_comm_bin.para4 > 0)
      {
        ...
        {  // ��̊p�x�̌v�Z�l�Ǝ��ۂ̒l�����킷����
          ...
          if (min_period < 0.1f)
            min_period = 0.1f;
          if (xv_data_d[HEAD_YAW].time < min_period)
            xv_data_d[HEAD_YAW].time = min_period;
        }
    ...
    case 'M': /*	motion (special action)	*/
      ...
      {
        if (_xv_comm_bin.para1 >= 0 && _xv_comm_bin.para2 >= 0 && _xv_comm_bin.para3 >= 0)
        {
          w = _xv_comm_bin.para1 * 100 + _xv_comm_bin.para2 * 10 + _xv_comm_bin.para3;
    ...
    case 'O':
      xv_mv.count = 0;
      accurate_walk_command(2, _xv_comm_bin.para1 * 10, _xv_comm_bin.para2 * 10, xp_mv_walk.accurate_step_z,
                            _xv_comm_bin.para3 * 2, xv_joy.walk_time);
      break;
    case 'U':
      accurate_walk_command(_xv_comm_bin.para1, _xv_comm_bin.para2 * 10, _xv_comm_bin.para3 * 10, xp_mv_walk.z,
                            _xv_comm_bin.para4 * 2, xv_joy.walk_time);
      break;
    case 'V':
      if (((flag_motion_accept == ON) || (flag_acc.fall == 0)) && (flag_servo_on))
      {
...
void joy_read(void)
{
  sscanf((char*)&rfmt[5], "%c%c%c%c%c%c", &xv_comm.cmd, &xv_comm.para1, &xv_comm.para2, &xv_comm.para3, &xv_comm.para4,
         &xv_comm.para5);
